...
However, for production maps, it's recommended to deploy your PMTiles tileset behind a cloud function (AWS Lambda, Cloudflare Worker, etc). The cloud function can receive normal requests for tiles by `/z/x/y` URL path, fetch the appropriate range of data from the PMTiles archive behind the scenes, and return it to the client. This has several advantages:
